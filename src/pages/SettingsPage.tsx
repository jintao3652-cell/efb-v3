...
import { clearNetworkCache } from "../lib/network-cache";
...
export function SettingsPage() {
  ...
  const cycle = airac.data?.cycleId;
  const navStatus = navigation.data;
  const navSourceName = navStatus?.source === "fenix" ? "Fenix" : "Little Navmap";

  const handleClear = async () => {
    await Promise.all([clearCache(), clearNetworkCache()]);
    for (const queryKey of [["weather"], ["current-airac-cycle"], ["xfly-airport"], ["vatsim-fir-boundaries"], ["vatsim-tracon-boundaries"], ["vatsim-live-snapshot"], ["vatglasses"], ["osm-airport-ground"]]) queryClient.removeQueries({ queryKey });
    setMessage("已清除航图、天气、机场、AIRAC 与网络响应缓存；飞行计划、导航数据库和设置已保留。");
  };
  ...
  return <div className="settings-page page-stack">
    ...
    <section className="panel setting-section">
      <div className="setting-title"><div><HardDrive className="accent-icon" size={22} /><div><h3>本地缓存</h3><p>航图 PDF、AIRAC、天气、机场和地图网络数据</p></div></div><strong>自动管理</strong></div>
    ...
    <section className="panel setting-section">
      ...
      <div className="database-source-choice">
        ...
        <button className={navStatus?.source === "fenix" ? "active" : ""} onClick={() => chooseDatabase("fenix")} disabled={navigationMutation.isPending}><strong>Fenix</strong><span>nd.db3：机场、航路、跑道、SID/STAR 和 AIRAC</span><Upload size={16} /></button>
      </div>
    ...
    <section className="panel setting-section">
      <div className="setting-title"><div><Database className="accent-icon" size={22} /><div><h3>航行数据</h3><p>{airac.data ? `${airac.data.provider} · 周期开始于 ${new Date(airac.data.cycleStartDate).toLocaleDateString("zh-CN")}` : airac.isError ? "当前周期不可用" : "正在检查 Navigraph FMS Data 周期"}</p></div></div><span className={`data-cycle ${cycle ? "" : "pending"}`}>{cycle ? `AIRAC ${cycle}` : "AIRAC --"}</span></div>
      <div className="setting-row"><div><strong>AIRAC 当前周期</strong><p>{airac.isError ? "无法连接周期服务，且本机没有可用缓存。" : cycle ? `周期 ${cycle} 可用于检测导航数据是否需要更新。` : "正在获取当前周期。"}</p></div><button className="button secondary" onClick={() => airac.refetch()} disabled={airac.isFetching}><RefreshCw className={airac.isFetching ? "spinning" : ""} size={16} />{airac.isFetching ? "正在检查" : "检查更新"}</button></div>
    </section>
  </div>;
}
