...
import { Archive, BookOpen, ChevronLeft, ChevronRight, Download, ExternalLink, FilePlus2, FileText, FolderOpen, RefreshCw, Search } from "lucide-react";
...
export function ChartsPage() {
  ...
  const localLibraryMutation = useMutation({ mutationFn: setLocalChartLibrary, onSuccess: () => { client.invalidateQueries({ queryKey: ["local-chart-library"] }); client.invalidateQueries({ queryKey: ["local-charts"] }); setError(""); } });
  const activeCharts = localLibrary.data?.ready ? (localCharts.data ?? []).map((chart) => ({ id: chart.id, airport: chart.airport, category: chart.category, title: chart.title, revision: chart.revision, cached: true })) : charts;
  const filtered = useMemo(() => activeCharts.filter((chart) => (category === "全部" || chart.category === category) && `${chart.airport} ${chart.title}`.toLowerCase().includes(query.toLowerCase())), [activeCharts, category, query]);
  const groupedLocalCharts = useMemo(() => {
    const groups = new Map<string, Chart[]>();
    for (const chart of filtered) groups.set(chart.airport, [...(groups.get(chart.airport) ?? []), chart]);
  ...
  const chooseLibrary = async (sourceType: "folder" | "zip") => {
    if (!isTauri()) { setError("浏览器模式不支持本地航图库。"); return; }
  ...
  const chooseLocalChart = async (chart: Chart) => {
    chooseChart(chart);
    try { const cachedPath = await openLocalChart(chart.id); setPdfSource(convertFileSrc(cachedPath)); setFileName(`${chart.title}.pdf`); } catch (reason) { setError(reason instanceof Error ? reason.message : "无法打开本地航图"); }
  };
  const importPdf = async () => {
    if (!isTauri()) { inputRef.current?.click(); return; }
    const sourcePath = await open({ multiple: false, directory: false, filters: [{ name: "PDF 航图", extensions: ["pdf"] }] });
    if (!sourcePath || Array.isArray(sourcePath)) return;
    try { const cachedPath = await cacheChartPdf(sourcePath, selected.id); setPdfSource(convertFileSrc(cachedPath)); setFileName(sourcePath.split(/[\\/]/).pop() ?? "航图.pdf"); setPageCount(0); setPageNumber(1); setError(""); } catch (reason) { setError(reason instanceof Error ? reason.message : "无法导入航图"); }
  };

  return <div className="chart-page split-page">
    <section className="panel chart-sidebar">
      <div className="chart-source-selector"><button className={source === "local" ? "active" : ""} onClick={() => setSource("local")}>本地航图库</button><button className={source === "chartfox" ? "active" : ""} onClick={() => setSource("chartfox")}>CHARTFOX</button></div>
      {source === "local" ? <>
        ...
        {localLibrary.data?.ready && <><div className="search-box"><Search size={18} /><input placeholder="搜索航图或机场" value={query} onChange={(event) => setQuery(event.target.value)} /></div><div className="chart-filter">{(["全部", "机场", "进场", "离场", "航路"] as const).map((item) => <button key={item} className={category === item ? "active" : ""} onClick={() => setCategory(item)}>{item}</button>)}</div><div className="chart-list">{localCharts.isFetching && <p className="chartfox-state">正在读取航图目录…</p>}{groupedLocalCharts.map(([airport, airportCharts]) => <div className="chart-airport-group" key={airport}><div className="chart-airport-heading"><strong>{airport}</strong><span>{airportCharts.length} 份航图</span></div>{airportCharts.map((chart) => <button key={chart.id} onClick={() => chooseLocalChart(chart)} className={selected.id === chart.id ? "selected" : ""}><FileText size={18} /><span><strong>{chart.title}</strong><small>{chart.category} · {chart.revision}</small></span><Download size={15} className="cached-icon" /></button>)}</div>)}</div></>}
      </> : <>
    ...
    <section className="chart-viewer"><div className="chart-viewer-header"><div><p className="eyebrow">{selected.airport} · {source === "chartfox" ? "CHARTFOX" : selected.category}</p><h2>{pdfSource ? fileName : selected.title}</h2></div><StatusBadge tone={pdfSource ? "success" : "warning"}>{pdfSource ? "本地已打开" : source === "chartfox" ? "在线索引" : "未打开"}</StatusBadge></div><input className="visually-hidden" ref={inputRef} type="file" accept="application/pdf" onChange={(event) => { const file = event.target.files?.[0]; if (file) loadPdf(file); }} />{pdfSource ? <div className="pdf-viewer"><Document file={pdfSource} onLoadSuccess={({ numPages }: { numPages: number }) => { setPageCount(numPages); setPageNumber(1); }} onLoadError={(reason) => setError(`无法读取 PDF：${reason instanceof Error ? reason.message : "文件加载失败"}`)} loading={<div className="pdf-loading">正在渲染航图…</div>}><Page pageNumber={pageNumber} width={760} renderTextLayer renderAnnotationLayer /></Document></div> : <ProcedurePreview chart={selected} />}<div className="chart-controls"><button className="button secondary" onClick={importPdf}><FilePlus2 size={17} />导入单份 PDF</button>{chartFoxUrl && <a className="button secondary" href={chartFoxUrl} target="_blank" rel="noreferrer"><ExternalLink size={16} />在 ChartFox 查看</a>}{pdfSource && <><button className="icon-button" disabled={pageNumber <= 1} onClick={() => setPageNumber((page) => page - 1)} aria-label="上一页"><ChevronLeft size={18} /></button><span>第 {pageNumber} / {pageCount || "–"} 页</span><button className="icon-button" disabled={pageCount === 0 || pageNumber >= pageCount} onClick={() => setPageNumber((page) => page + 1)} aria-label="下一页"><ChevronRight size={18} /></button></>}<span className="chart-cache-note"><BookOpen size={15} />{isTauri() ? "打开的航图将复制到本机缓存" : "浏览器模式仅临时预览"}</span></div>{error && <p className="form-error">{error}</p>}<div className="chart-meta"><span>来源：{source === "chartfox" ? "ChartFox" : localLibrary.data?.sourceType === "zip" ? "本地 ZIP" : "本地文件夹"}</span><span>格式：PDF</span><span>请遵守数据源的使用条款</span></div></section>
  </div>;
}
