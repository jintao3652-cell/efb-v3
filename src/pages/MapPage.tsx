...
import { applyActiveTraconControllers, loadGlobalAirspace, loadVatsimControllers, loadVatsimPilots, type VatsimPilot } from "../lib/airspace";
import { airports } from "../lib/data";
import { loadOsmAirportGround } from "../lib/osm-airport";
...
const mapLayerIds: Record<LayerKey, string[]> = {
  ...
  airportGround: ["osm-apron-fill", "osm-apron-line", "osm-terminal-fill", "osm-terminal-line", "osm-runway-casing", "osm-runway-line", "osm-taxiway-casing", "osm-taxiway-line", "osm-runway-labels", "osm-taxiway-labels", "osm-stand-points", "osm-stand-labels"],
...
export function MapPage() {
  ...
  const globalAirspace = useQuery({ queryKey: ["vatsim-boundaries"], queryFn: loadGlobalAirspace, retry: 1, staleTime: 60 * 60_000 });
  const vatsimControllers = useQuery({ queryKey: ["vatsim-controllers"], queryFn: loadVatsimControllers, retry: 1, refetchInterval: 60_000, staleTime: 45_000 });
  const vatsimPilots = useQuery({ queryKey: ["vatsim-pilots"], queryFn: loadVatsimPilots, retry: 1, refetchInterval: 15_000, staleTime: 12_000 });
  const airportGroundBounds = useMemo(() => ({ west: Number(viewport.west.toFixed(2)), south: Number(viewport.south.toFixed(2)), east: Number(viewport.east.toFixed(2)), north: Number(viewport.north.toFixed(2)) }), [viewport.west, viewport.south, viewport.east, viewport.north]);
  const airportGround = useQuery({ queryKey: ["osm-airport-ground", airportGroundBounds], queryFn: () => loadOsmAirportGround(airportGroundBounds), enabled: layers.airportGround && viewport.zoom >= 12, retry: 1, staleTime: 10 * 60_000 });
  ...
  const traconCoverage = useMemo(() => globalAirspace.data ? applyActiveTraconControllers(globalAirspace.data.tracon, vatsimControllers.data ?? []) : undefined, [globalAirspace.data, vatsimControllers.data]);
  const selectedPilot = useMemo(() => (vatsimPilots.data ?? []).find((pilot) => pilot.callsign === selectedCallsign), [vatsimPilots.data, selectedCallsign]);
  ...
  useEffect(() => {
    ...
    instance.on("load", () => {
      ...
      instance.addLayer({ id: "fir-fill", type: "fill", source: "fir", paint: { "fill-color": "#7957d5", "fill-opacity": .055 } }); instance.addLayer({ id: "fir-line", type: "line", source: "fir", paint: { "line-color": "#a98cff", "line-width": 1.25, "line-opacity": .8 } }); instance.addLayer({ id: "fir-labels", type: "symbol", source: "fir", minzoom: 3, layout: { "text-field": ["get", "id"], "text-size": 11, "text-allow-overlap": false }, paint: { "text-color": "#c8baff", "text-halo-color": "#071827", "text-halo-width": 1.5 } });
      instance.addLayer({ id: "tracon-fill", type: "fill", source: "tracon", minzoom: 5, layout: { visibility: "none" }, paint: { "fill-color": ["case", ["==", ["get", "active"], true], "#18c8f7", "#426f82"], "fill-opacity": ["case", ["==", ["get", "active"], true], .8, .7] } }); instance.addLayer({ id: "tracon-active-glow", type: "line", source: "tracon", minzoom: 5, filter: ["==", ["get", "active"], true], layout: { visibility: "none" }, paint: { "line-color": "#40d9ff", "line-width": 9, "line-opacity": .24, "line-blur": 3 } }); instance.addLayer({ id: "tracon-separator-casing", type: "line", source: "tracon", minzoom: 5, layout: { visibility: "none" }, paint: { "line-color": "#04131f", "line-width": ["case", ["==", ["get", "active"], true], 6.5, 4.5], "line-opacity": .92 } }); instance.addLayer({ id: "tracon-line", type: "line", source: "tracon", minzoom: 5, layout: { visibility: "none" }, paint: { "line-color": ["case", ["==", ["get", "active"], true], "#d5f8ff", "#8fc8da"], "line-width": ["case", ["==", ["get", "active"], true], 3.4, 2.2], "line-opacity": ["case", ["==", ["get", "active"], true], 1, .9] } }); instance.addLayer({ id: "tracon-labels", type: "symbol", source: "tracon", minzoom: 5, filter: ["==", ["get", "active"], true], layout: { visibility: "none", "text-field": ["get", "activeCallsign"], "text-size": 13, "text-letter-spacing": .08, "text-allow-overlap": true, "text-ignore-placement": true }, paint: { "text-color": "#effcff", "text-halo-color": "#062033", "text-halo-width": 2.2 } });
  ...
  useEffect(() => { if (!mapReady || !map.current || !globalAirspace.data) return; (map.current.getSource("fir") as maplibregl.GeoJSONSource).setData(globalAirspace.data.fir as never); }, [mapReady, globalAirspace.data]);
  ...
  useEffect(() => { if (!mapReady || !map.current) return; (map.current.getSource("traffic") as maplibregl.GeoJSONSource).setData(trafficAsFeatures(vatsimPilots.data ?? [])); }, [mapReady, vatsimPilots.data]);
  useEffect(() => {
    if (!mapReady || !map.current) return;
  ...
  return <div className="map-page"><div className="map-canvas" ref={container} />
    ...
    <aside className={`map-layers ${layersOpen ? "" : "collapsed"}`}><div className="map-layers-heading"><div><Layers size={18} /><strong>地图图层</strong></div><button onClick={() => setLayersOpen((open) => !open)} aria-label={layersOpen ? "横向收起图层面板" : "展开图层面板"} title={layersOpen ? "横向收起" : "展开图层"}>{layersOpen ? <PanelRightClose size={18} /> : <PanelRightOpen size={18} />}</button></div>{layersOpen && <><div className="map-style-selector" role="group" aria-label="地图图源"><button className={mapStyle === "standard" ? "active" : ""} onClick={() => setMapStyle("standard")}><MapIcon size={14} />标准地图</button><button className={mapStyle === "satellite" ? "active" : ""} onClick={() => setMapStyle("satellite")}><Satellite size={14} />卫星地图</button></div><div className="layer-list">{layerOptions.map(({ key, label }) => <label className="layer-toggle" key={key}><span>{label}</span><input type="checkbox" checked={layers[key]} onChange={() => setLayers((current) => ({ ...current, [key]: !current[key] }))} /><i /></label>)}</div><p>{navigationData.isFetching ? "正在更新当前可视范围内的航空要素…" : dataStatus}</p><p>{viewport.zoom < 12 ? "放大至 Zoom 12 加载 OSM 机场地面设施" : airportGround.isFetching ? "正在加载 OSM 跑道、滑行道和机坪…" : airportGround.data ? `OSM 机场地面设施：${airportGround.data.features.length} 个要素` : "OSM 机场地面设施已启用"}</p><p>VATSIM 实时席位：{vatsimControllers.data?.length ?? 0} · TRACON 每 60 秒刷新</p>{globalAirspace.isError && <p className="map-error">FIR / TRACON 边界加载失败。</p>}{vatsimControllers.isError && <p className="map-error">VATSIM 实时席位加载失败，当前仅显示静态边界。</p>}{layers.airportGround && viewport.zoom >= 12 && airportGround.isError && <p className="map-error">OSM 机场地面数据加载失败，请稍后重试。</p>}{layers.weather && !openWeatherKey && <p className="map-error">未配置 VITE_OPENWEATHER_API_KEY，无法加载降水雷达。</p>}{navigationData.isError && <p className="map-error">{navigationData.error.message}</p>}</>}</aside>
