...
const mapLayerIds: Record<LayerKey, string[]> = {
  ...
  airports: ["navigation-airports", "navigation-airport-labels"], airspace: ["airspace-fill", "airspace-line"], traffic: ["traffic-points"], weather: ["weather-radar"],
};
...
export function MapPage() {
  ...
  return <div className="map-page"><div className="map-canvas" ref={container} />
    ...
    <aside className="map-layers"><div className="map-layers-heading"><div><Layers size={18} /><strong>地图图层</strong></div><button onClick={() => setLayersOpen((open) => !open)} aria-label="折叠图层"><X className={layersOpen ? "" : "map-panel-closed"} size={17} /></button></div>{layersOpen && <><div className="layer-list">{layerOptions.map(({ key, label }) => <label className="layer-toggle" key={key}><span>{label}</span><input type="checkbox" checked={layers[key]} onChange={() => setLayers((current) => ({ ...current, [key]: !current[key] }))} /><i /></label>)}</div><p>{navigationData.isFetching ? "正在更新当前可视范围内的航空要素…" : dataStatus}</p>{layers.weather && !openWeatherKey && <p className="map-error">未配置 VITE_OPENWEATHER_API_KEY，无法加载降水雷达。</p>}{navigationData.isError && <p className="map-error">{navigationData.error.message}</p>}</>}</aside>
