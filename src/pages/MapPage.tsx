...
export function MapPage() {
  ...
  useEffect(() => {
    ...
    instance.on("load", () => {
      ...
      // 标签显示呼号（灰=stale、黄=真实在线）。徽章压在机场齿轮符号上层恒显
      // （allow-overlap），否则先占位的机场符号会把徽章碰撞挤掉、只剩呼号标签。
      instance.addLayer({ id: "atc-icons", type: "symbol", source: "atc-icons", minzoom: 8, layout: { "icon-image": ["get", "icon"], "icon-size": ["interpolate", ["linear"], ["zoom"], 8, .8, 12, 1], "icon-allow-overlap": true, "icon-ignore-placement": true, "icon-padding": 2 }, paint: { "icon-opacity": ["match", ["get", "stale"], 1, .45, 1] } });
      instance.addLayer({ id: "atc-icon-labels", type: "symbol", source: "atc-icons", minzoom: 9.5, layout: { "text-field": ["get", "label"], "text-size": 10, "text-offset": [0, 1.1], "text-anchor": "top", "text-allow-overlap": true, "text-ignore-placement": true, "text-optional": true }, paint: { "text-color": ["match", ["get", "stale"], 1, "#9fb0bd", "#ffe08a"], "text-halo-color": "#1c1403", "text-halo-width": 1.6 } });
