...
export function WeatherPage() {
  ...
  return <div className="weather-page page-stack"><div className="page-actions"><div><h2>天气与通告</h2><p>METAR / TAF 使用网络优先、本地缓存回退。</p></div><div className="station-input"><input value={station} maxLength={4} onChange={(event) => setStation(event.target.value.toUpperCase())} /><button className="button primary" onClick={() => weather.refetch()} disabled={!online || weather.isFetching}><RefreshCw size={16} className={weather.isFetching ? "spinning" : ""} />刷新</button></div></div>
    ...
    <div className="weather-grid"><section className="panel"><p className="eyebrow">TAF 预报 · {provider}</p><h3>{station} 航空例行天气预报</h3><p className="forecast">{report.taf}</p>{report.previewUrl && <a className="text-link" href={report.previewUrl} target="_blank" rel="noreferrer"><ExternalLink size={14} />在 metar-taf.com 预览</a>}<small>TAF 由当前气象数据源返回；请在飞行前复核官方资料。</small></section><section className="panel"><p className="eyebrow">NOTAM</p><h3>运行通告</h3><div className="notam"><span>RWY</span><p>暂无影响当前计划的跑道关闭通告。</p></div><div className="notam"><span>OPS</span><p>请在飞行前向官方来源确认全部 NOTAM。</p></div></section></div>
  </div>;
}
