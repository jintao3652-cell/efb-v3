...
fn value_text(value: Option<&Value>) -> String {
    value.and_then(|value| value.as_str().map(str::to_string).or_else(|| value.as_f64().map(|number| number.to_string()))).unwrap_or_default()
}
...
fn coordinate(value: Option<&Value>, latitude: bool) -> Option<f64> {
    ...
    let numeric = raw.trim_start_matches(['N', 'S', 'E', 'W', '+', '-']).parse::<f64>().ok()?;
...
fn simbrief_route_points(payload: &Value) -> Vec<FlightRoutePoint> {
    let fixes = payload.pointer("/navlog/fix").or_else(|| payload.pointer("/navlog/fixes")).and_then(Value::as_array);
    ...
    for fix in fixes.iter().filter_map(Value::as_object) {
        ...
        let ident = ["ident", "fix", "name", "id"].iter().map(|key| value_text(fix.get(*key))).find(|value| !value.is_empty()).unwrap_or_else(|| "航路点".to_string());
        if points.last().is_some_and(|point: &FlightRoutePoint| (point.latitude - latitude).abs() < 0.00001 && (point.longitude - longitude).abs() < 0.00001) { continue; }
        points.push(FlightRoutePoint { name: value_text(fix.get("name")), ident, latitude, longitude });
    }
    points
}
