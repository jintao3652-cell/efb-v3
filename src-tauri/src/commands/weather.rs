...
fn json_text(value: Option<&Value>) -> String {
    value.and_then(|item| item.as_str().map(ToOwned::to_owned).or_else(|| item.as_i64().map(|number| number.to_string())).or_else(|| item.as_f64().map(|number| number.round().to_string()))).unwrap_or_default()
}

fn available(value: Option<&Value>) -> String {
    let result = json_text(value);
    if result.is_empty() || matches!(result.as_str(), "999998" | "999999") { "--".to_string() } else { result }
}

fn nmc_taf_summary(station: &str, taf: &Value) -> String {
    format!("TAF {station} 有效期 {} 至 {} · 风 {}°/{} m/s · 能见度 {} · 云况 {}", available(taf.get("VALID_START_TIME")), available(taf.get("VALID_END_TIME")), available(taf.get("MEAN_WIND_DIRECTION")), available(taf.get("MEAN_WIND_SPEED")), available(taf.get("PREVAG_VISIBILITY")), available(taf.get("CLOUD_INFO")))
}

async fn nmc_weather_report(station: &str) -> Result<WeatherReport, String> {
    let client = reqwest::Client::builder().timeout(std::time::Duration::from_secs(12)).user_agent("SkyBoard-EFB/1.0").build().map_err(|_| "无法创建气象请求".to_string())?;
    ...
    let taf = match client.get(taf_url).send().await {
        Ok(response) if response.status().is_success() => response.json::<Value>().await.ok().map(|value| nmc_taf_summary(station, &value)).unwrap_or_else(|| "TAF 暂不可用".to_string()),
        _ => "TAF 暂不可用".to_string(),
    };
    ...
    let visibility_display = if visibility == "--" { "CAVOK / --".to_string() } else { format!("{visibility} m") };
    Ok(WeatherReport {
        station: station.to_string(),
        raw: format!("METAR {station} {} {}° {} m/s VIS {} T {}°C / {}°C Q{} hPa", available(metar.get("time")), wind_direction, wind_speed, visibility_display, temperature, dewpoint, qnh),
        wind: format!("{wind_direction}° / {wind_speed} m/s"),
        visibility: visibility_display,
        temperature: if temperature == "--" { "--".to_string() } else { format!("{temperature}°C / {dewpoint}°C") },
        qnh: if qnh == "--" { "--".to_string() } else { format!("{qnh} hPa") },
...
async fn awc_weather_report(station: &str) -> Result<WeatherReport, String> {
    ...
    let taf = match client.get(taf_url).send().await {
        Ok(response) => match response.error_for_status() { Ok(response) => response.json::<Value>().await.ok().and_then(|payload| payload.as_array().and_then(|items| items.first()).and_then(|item| item.get("rawTAF")).and_then(Value::as_str).map(ToOwned::to_owned)).unwrap_or_else(|| "Aviation Weather 未返回 TAF。".to_string()), Err(_) => "Aviation Weather 未返回 TAF。".to_string() },
        Err(_) => "Aviation Weather 未返回 TAF。".to_string(),
    };
    let wind_direction = match json_text(metar.get("wdir")).as_str() { "" => "VRB".to_string(), "VRB" => "VRB".to_string(), value => format!("{value}°") };
    ...
    Ok(WeatherReport { station: station.to_string(), raw: metar.get("rawOb").and_then(Value::as_str).unwrap_or("原始 METAR 不可用").to_string(), wind: format!("{wind_direction} / {wind_speed} kt"), visibility: available(metar.get("visib")), temperature: if temperature.is_empty() { "--".to_string() } else { format!("{temperature}°C / {}°C", if dewpoint.is_empty() { "--" } else { &dewpoint }) }, qnh: metar.get("altim").and_then(Value::as_f64).map(|value| format!("{value:.0} hPa")).unwrap_or_else(|| "--".to_string()), observed_at: metar.get("reportTime").and_then(Value::as_str).unwrap_or("实时").to_string(), source: "实时".to_string(), taf, preview_url: Some(format!("https://metar-taf.com/site/preview-airport?id={station}&nightmode=false")) })
}
...
pub async fn get_weather(station: String) -> Result<WeatherReport, String> {
    ...
    if station.starts_with('Z') {
        match nmc_weather_report(&station).await { Ok(report) => return Ok(report), Err(error) => errors.push(error) }
    }
    match awc_weather_report(&station).await { Ok(report) => return Ok(report), Err(error) => errors.push(error) }
    if let Ok(api_key) = std::env::var("OPENWEATHER_API_KEY") {
        match openweather_report(&station, &api_key).await { Ok(report) => return Ok(report), Err(error) => errors.push(error) }
    }
    Err(format!("气象获取失败：{}", errors.join("；")))
}
